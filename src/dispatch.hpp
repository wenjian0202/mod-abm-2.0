...
/// \details The cost is defined as the total time taken to drop each of the trips based on the current waypoints.
...
/// \param system_time_s The system time.
/// \tparam router_func The router func that finds path between two poses.
/// \return A pair. True if the trip can be inserted, together with the pick up time. False otherwise.
template <typename RouterFunc>
...
/// Otherwise, result is true. The cost_s is the additional cost in seconds required to serve this trip,
