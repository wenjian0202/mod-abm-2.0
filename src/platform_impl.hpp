      ...
      demand_generator_func_(std::move(_demand_generator_func))
{
    ...
    Vehicle vehicle{0, {fleet_config.initial_lon, fleet_config.initial_lat}, fleet_config.veh_capacity, 0, {}, 0.0, 0.0};
...
void Platform<RouterFunc, DemandGeneratorFunc>::run_cycle()
{
    ...
    bool in_main_simulation =
        system_time_s_ >= platform_config_.simulation_config.warmup_duration_s && system_time_s_ < platform_config_.simulation_config.warmup_duration_s + platform_config_.simulation_config.simulation_duration_s;
    ...
    for (auto &request : requests)
    {
        ...
        fmt::print("[DEBUG] Trip #{} requested at T = {}, from origin ({}, {}) to destination ({}, {}):\n", trip.id, trip.request_time_s, trip.origin.lon, trip.origin.lat, trip.destination.lon, trip.destination.lat);
    }
...
void Platform<RouterFunc, DemandGeneratorFunc>::create_report()
{
    ...
    fmt::print(" - Simulation Config: simulation_duration = {}s ({}s warm-up + {}s main + {}s wind-down).\n",
               total_simulation_time_s,
               platform_config_.simulation_config.warmup_duration_s,
               platform_config_.simulation_config.simulation_duration_s,
               platform_config_.simulation_config.winddown_duration_s);
    ...
    fmt::print("# Trips\n");
    fmt::print(" - Total Trips: requested = {} (of which {} dispatched [{}%] + {} walked away [{}%]).\n",
               trip_count,
               dispatched_trip_count,
    ...
    fmt::print("# Vehicles\n");
    fmt::print(" - Distance: average_distance_traveled = {}m. average_distance_traveled_per_hour = {}m.\n",
               total_dist_traveled_m / vehicles_.size(),
               total_dist_traveled_m / vehicles_.size() / platform_config_.simulation_config.simulation_duration_s * 3600);
